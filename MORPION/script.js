...
function choiseCase(clicked_id) {
    ...
    if (clicked % 2 == 0) {
        $('#' + clicked_id).html( < 'p class = "animated bounceIn"style = "font-size:2em;color:#4286f4;" >' + player.team + ' </p>);
            grid.splice(grid.indexOf(clicked_id), 1); player.arr.push(clicked_id); findElement(player); finished(); $('.player-turn').css({ "border-bottom": "none" }) $('.cpu-turn').css({ "border-bottom": "3px solid #e29126" }) $('#' + clicked_id).prop("disabled", true); setTimeout(computer, 1300);
        }
        choice = 1,
            clicked++;
    }
    ...
    function teamchoice(clicked_id) {
        ...
        $('.layer').fadeOut(400),
            player.team = document.getElementsById(clicked_id).innerHTML
        if (player - turn === 'X') {}
    }
